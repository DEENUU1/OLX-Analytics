...
from base.data.parser import ParamsData, ParseData, Parser, convert_datetime_to_date


@pytest.fixture
def data_to_parse() -> Any:
...
def test_parser_data_parser_return_params_data_objects(data_to_parse) -> None:
    ...
    params_data_1 = ParamsData(
        key="price_per_m", name="Cena za m²", value="4474.58 zł/m²", value_key="4474.58"
    )
    assert parser.data_parser()[0].params[0] == params_data_1
    assert (
        parser.data_parser()[0].url
        == "https://www.olx.pl/d/oferta/mieszkanie-blok-59m2-CID3-IDToKpO.html"
    )
